...
#SBATCH --mem={slurm_args.get('memory', '2G')}
...
if task_id > len(combinations):
    print('Array task ID {{}} exceeds number of combinations {{}}'.format(task_id, len(combinations)))
    sys.exit(1)

combination = combinations[task_id - 1]  # Convert to 0-indexed

# Create combination name
combo_name = '{grid_config.base_prefix}_n{{}}_prev{{}:.3f}_snps{{}}_causal{{}}'.format(combination['cohort_size'], combination['prevalence'], combination['total_snps'], combination['causal_snps'])
combo_dir = '{grid_config.grid_output_dir}/' + combo_name

print('Processing combination {{}}: {{}}'.format(task_id, combo_name))
print('  Cohort: {{}} ({{}} cases, {{}} controls)'.format(combination['cohort_size'], combination['num_cases'], combination['num_controls']))
print('  Prevalence: {{}:.3f}'.format(combination['prevalence']))
print('  SNPs: {{}} ({{}} causal)'.format(combination['total_snps'], combination['causal_snps']))
